...
SCRIPT = os.path.join(ROOT, "scripts", "image_ops.py")
...
def main():
    ...
    draft_id = ((create_draft().get("output") or {}).get(
        "draft_id")) if isinstance(create_draft(), dict) else None
    if not draft_id:
    ...
    payload = {
        ...
        "transition": combo_animation_type,
