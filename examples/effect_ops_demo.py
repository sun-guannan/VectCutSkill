...
SCRIPT = ROOT / "scripts/effect_ops.py"
...
def create_draft():
    ...
    req = urllib.request.Request(
        url=url,
        data=json.dumps({"name": "effect_demo", "width": 1080, "height": 1920}, ensure_ascii=False).encode("utf-8"),
        method="POST",
...
def main():
    ...
    names = load_items(ROOT / "references/enums/scene_effect_types.json") + load_items(ROOT / "references/enums/character_effect_types.json")
    if not names:
        print("No effect types found in enum files.")
        sys.exit(1)

    effect_type = random.choice(names)
    print(f"Use effect_type: {effect_type}")
