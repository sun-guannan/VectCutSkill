...
pick_random_intro_animation_type() {
  (
    ...
    grep -o '"name"[[:space:]]*:[[:space:]]*"[^"]*"' "${ROOT}/references/enums/transition_types.json" || true
...
PAYLOAD='{"image_url":"https://pic1.imgdb.cn/item/68ba8fc058cb8da5c8801ab0.png","start":0,"end":5,"width":1920,"height":1080,"transform_x":0.2,"transform_y":0.2,"scale_x":1,"scale_y":1,"track_name":"video_main","relative_index":99,"intro_animation":"'"$intro_animation"'","intro_animation_duration":0.5,"outro_animation":"'"$outro_animation"'","outro_animation_duration":0.5,"transition":"'"$combo_animation"'","transition_duration":0.5,"mask_type":"'"$mask_type"'","mask_center_x":0.5,"mask_center_y":0.5,"mask_size":0.7,"mask_rotation":45,"mask_feather":2,"mask_invert":true,"mask_rect_width":8,"mask_round_corner":10}'
RES="$("${ROOT}/scripts/image_ops.sh" add_image "$PAYLOAD")"
echo "add_image => $RES"
