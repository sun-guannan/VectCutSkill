...
def _normalize_token(s):
    return "".join(re.findall(r"[\u4e00-\u9fffA-Za-z0-9]+", s or ""))
...
def asr_601e98ed739a43b5a310a17e327fbe01(segments, draft_id):
    all_word_segments = []
    ...
    for item in normalized_segments or []:
        ...
        try:
            s_start = float(item.get("start"))
        except Exception:
            s_start = None
        try:
            s_end = float(item.get("end"))
        except Exception:
            s_end = None
        ...
        try:
            ...
            next_start = None
        if isinstance(s_end, (int, float)) and isinstance(next_start, (int, float)) and next_start > s_end:
            s_end = next_start

        missing_time = False
        for w in words:
            t = str((w.get("word") or w.get("text") or ""))
            ts = w.get("start_time")
            te = w.get("end_time")
            try:
                ts = float(ts)
            ...
            tokens.append(t)
            times.append((ts, te))
        if missing_time and isinstance(s_start, (int, float)) and isinstance(s_end, (int, float)):
            n = len(tokens) or 1
            times = []
            for i in range(n):
                cs = s_start + (s_end - s_start) * (i / n)
                ce = s_start + (s_end - s_start) * ((i + 1) / n)
                times.append((cs, ce))
        else:
            times = [(
                ts if isinstance(ts, (int, float)) else 0.0,
                te if isinstance(te, (int, float)) else (ts if isinstance(ts, (int, float)) else 0.0)
            ) for ts, te in times]
        if times and isinstance(next_start, (int, float)):
            ts_last, te_last = times[-1]
            if not isinstance(te_last, (int, float)) or next_start > te_last:
                times[-1] = (ts_last if isinstance(ts_last, (int, float)) else 0.0, next_start)
        if times:
            for j in range(1, len(times)):
                ...
                if isinstance(pe, (int, float)) and isinstance(cs, (int, float)):
                    if pe < cs:
                        times[j-1] = (ps if isinstance(ps, (int, float)) else 0.0, cs)
                    else:
                        times[j] = (pe, ce if isinstance(ce, (int, float)) else pe)
        
        norm_text, pos_map = _build_norm_map(full_text)
        norm_idx = 0
        orig_idx = 0
        yi = -0.75
        wi = 0
        for t, (ts, te) in zip(tokens, times):
            nt = _normalize_token(t)
            if nt:
                p = norm_text.find(nt, norm_idx)
                if p >= 0:
                    cs = pos_map[p] if p < len(pos_map) else orig_idx
                    end_norm = p + len(nt)
                    ce = (pos_map[end_norm - 1] + 1) if (end_norm - 1) < len(pos_map) else cs
                    norm_idx = end_norm
                    orig_idx = ce
                else:
                    l = len(nt)
                    ...
                    ce = cs + l
                    orig_idx = ce
                    norm_idx = min(len(pos_map), norm_idx + l)
            else:
                l = len(t) or 1
                cs = orig_idx
                ce = cs + l
            ...
            all_word_segments.append({
                ...
                "char_end": ce,
                "y": yi,
                "unique_id": f"s{si}_w{wi}"
            })
            wi += 1
        si += 1
    inputs = {
        "all_word_segments": all_word_segments,
        "highlight_color": "#00ff4b",
    ...
    {
      "type": "for",
      "id": "loop_flat_karaoke",
      "index": 0,
      "in": "${all_word_segments}",
      "loop": [
        {
          "type": "action",
          "id": "add_text_segment",
          ...
          "params": {
            ...
            "start": "${item.start}",
            "end": "${item.end}",
            "track_name": "karaoke_${item.unique_id}",
            "font_size": 12.0,
            "font": "烟波宋",
...
# print(asr_601e98ed739a43b5a310a17e327fbe01(segemnts, "123"))

def apply_template(segments, draft_id):
    return asr_601e98ed739a43b5a310a17e327fbe01(segments, draft_id)
