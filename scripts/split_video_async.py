...
SUBMIT_PATH = "/process/split_video/submit_task/submit_split_video_task"
DEFAULT_STATUS_PATH = "/process/split_video/submit_task/task_status"
...
def _validate_payload(payload):
    ...
    start_seconds = _parse_time_to_seconds(payload["start"])
    end_seconds = _parse_time_to_seconds(payload["end"])
...
def run(payload, status_path=DEFAULT_STATUS_PATH, poll_interval=2.0, timeout=600):
    token = _ensure_token()
...
def main():
    parser = argparse.ArgumentParser(
        description="Submit async split-video task and poll until done."
    )
    parser.add_argument(
        "payload_json",
        help="Path to request payload JSON for submit_split_video_task",
    )
    parser.add_argument("--output", default="split_video_result.json")
