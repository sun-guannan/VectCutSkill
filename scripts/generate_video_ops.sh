...
if [[ "$ACTION" == "generate_video" ]]; then
  ...
  FRAMERATE="$(extract_json_string framerate)"
  [[ -z "$DRAFT_ID" ]] && echo '{"success":false,"error":"draft_id is required","output":""}' && exit 0
  BODY="{\"draft_id\":\"${DRAFT_ID}\""
  [[ -n "$RESOLUTION" ]] && BODY+=" ,\"resolution\":\"${RESOLUTION}\""
  [[ -n "$FRAMERATE" ]] && BODY+=" ,\"framerate\":\"${FRAMERATE}\""
  BODY+="}"
  post_json generate_video "$BODY"
  echo
  exit 0
fi
...
if [[ "$ACTION" == "task_status" ]]; then
  ...
  [[ -z "$TASK_ID" ]] && echo '{"success":false,"error":"task_id is required","output":""}' && exit 0
  post_json task_status "{\"task_id\":\"${TASK_ID}\"}"
  echo
  exit 0
fi
...
if [[ "$ACTION" == "render_wait" ]]; then
  ...
  RESOLUTION="$(extract_json_string resolution)"
  FRAMERATE="$(extract_json_string framerate)"
  MAX_POLL_RAW="$(extract_json_string max_poll)"
  POLL_INTERVAL_RAW="$(extract_json_string poll_interval)"
  MAX_POLL="${MAX_POLL_RAW:-30}"
  POLL_INTERVAL="${POLL_INTERVAL_RAW:-2}"

  if [[ -z "$TASK_ID" ]]; then
    [[ -z "$DRAFT_ID" ]] && echo '{"success":false,"error":"draft_id or task_id is required","output":""}' && exit 0
    GEN_PAYLOAD="{\"draft_id\":\"${DRAFT_ID}\""
    [[ -n "$RESOLUTION" ]] && GEN_PAYLOAD+=" ,\"resolution\":\"${RESOLUTION}\""
    [[ -n "$FRAMERATE" ]] && GEN_PAYLOAD+=" ,\"framerate\":\"${FRAMERATE}\""
    GEN_PAYLOAD+="}"
  ...
  for ((i=0; i<MAX_POLL; i++)); do
    ...
    STATUS="$(printf '%s' "$LAST" | python3 -c 'import json,sys;print(((json.load(sys.stdin).get("output") or {}).get("status")) or "")')"
    RESULT_URL="$(printf '%s' "$LAST" | python3 -c 'import json,sys;print(((json.load(sys.stdin).get("output") or {}).get("result")) or "")')"
    if [[ "$STATUS" == "SUCCESS" && -n "$RESULT_URL" ]]; then
      echo "$LAST"
      exit 0
    fi
    if [[ "$STATUS" == "FAILURE" || "$STATUS" == "FAILED" ]]; then
      echo "{\"success\":false,\"error\":\"Render failed\",\"output\":${LAST:-"\"\""}}"
      exit 0
    fi
    sleep "$POLL_INTERVAL"
  done
  echo "{\"success\":false,\"error\":\"Render polling timeout\",\"output\":${LAST:-"\"\""}}"
  exit 0
fi
